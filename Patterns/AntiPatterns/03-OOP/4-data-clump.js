...
const getCountryCode = (name) => Object.keys(countries).find((key) => countries[key] === name);

const getAreaCode = (name) => Object.keys(areas).find((key) => areas[key] === name);
...
class Person {
  ...
  call() {
    // мы вызываем parsePhone и получает country, area, number
    const [country, area, number] = this.parsePhone();
    // отправляем их на валидацию
    if (!this.isValid(country, area, number)) {
      throw new Error('Invalid phone number');
    }
    // фция prepareCommand нахидится вне нашего класса Person
    // То-есть суть в том что эти country, area, number в троем гуляют от метода к методу ! Правильно бы сделать класс PhoneNumber и уже с ним работать
