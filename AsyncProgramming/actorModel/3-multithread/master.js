...
// Мы заменели библиотеку child_process на worker_threads
...
class MasterSystem {
  static start(name, count = 1) {
    ...
    for (let i = 0; i < count; i++) {
      const actor = new Worker('./system.js'); // мы вместо fork делаем new Worker
      console.log({ actor });
      MasterSystem.subscribe(actor);
      ready.push(actor);
      instances.push(actor);
      actor.postMessage({ command: 'start', name }); // вместо actor.send мы делаем postMessage
    }
  }
