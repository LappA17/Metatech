...
// Так как у нас диферры имеют внутри подписку на события, по-этому будем использовать EventEmitter что бы внутри можно было несколько раз подписаться на каждый из событий
// class Deferred extends EventEmitter так как мы унаследовались то у нас будет вся фциональность ЕвентЕмиттера
// вызываем super() что бы создался констурктор ЕвентЕмиттера - он там создатс своим структурки данных в которых будет хранить обработчики событий
...
class Deferred extends EventEmitter {
  ...
  // теперь вместо проверки есть ли у нас свойство onFail или onDone мы просто делаем emit метода 'fail' или 'done' и передаём значение(value) которое прийдет всем подписчикам на событие fail
