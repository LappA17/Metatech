...
const wrapAsync = callback => setTimeout(callback, Math.floor(Math.random() * 1000));
...
Promise.resolve()
  ...
  .then(() => Promise.all([doQuery('select * from cities'), httpGet('http://kpi.ua')]))
